 ...
 /* TIM Configuration */
  ...
  while (1){ 
			...
			if (UBPressed==1) { 
				...
				factor = (factor+1)%2;
				TIM_PrescalerConfig(TIM3, PrescalerValue*(factor+1), TIM_PSCReloadMode_Immediate);
...
void TIM3_Config(void) 
{
	...
	TIM_TimeBaseStructure.TIM_Period=250000; // 65535need to be larger than CCR1_VAL, has no effect on the Output compare event.
